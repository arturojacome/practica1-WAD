        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<div class='container'>");
                out.println(
                            ...
                            +                       "<li class='nav-item'>"
                            +                           "<a class='nav-link active' href='MostrarDatosCategoria'>Mostrar datos Categoría</a>"
                            +                       "</li>"
            ...
            out.println("<div class='card text-white  mb-3' style='width: 30rem; margin: auto; margin-top: 20px;'>");
                    out.println("<div class='card-header ' style=' background-color: black; text-align: center;'>");
                        out.println("<h1>" + "Actualizar Categoría" + "</h1>");
                    out.println("</div>");
                    ...
                    if (dto != null) {          
                        ...
                        out.println("<form name='frmDatos' method='post' action='ActualizaCategoria'>");
                            out.println("<div style='margin-top: 15px;' class='form-group'>");
                            
                                out.println("<label>Clave:</label>");
                                out.println("<input style='margin-top: 15px; margin-bottom: 10px;' type='text' name='txtClave' class='form-control' value='" + dto.getEntidad().getIdCategoria()+ "' readonly>");
                                
                                out.println("<label>Nombre:</label>");
                                out.println("<input style='margin-top: 15px; margin-bottom: 10px;' type='text' name='txtNombre' class='form-control' value='" + dto.getEntidad().getNombreCategoria()+ "' required>");
                                
                                out.println("<label>Descripción:</label>");
                                out.println("<input style='margin-top: 15px; margin-bottom: 20px;' type='text' name='txtDescripcion' class='form-control' value='"+dto.getEntidad().getDescripcionCategoria()+"' required>");
                    } else {
                        
                                out.println("Sin conincidencias.");
                            }
                    
                        out.println("<input type='submit' value='Actualizar' href='MostrarDatosCategoria' class='btn btn-primary'/>");
                        out.println("</form>");      
